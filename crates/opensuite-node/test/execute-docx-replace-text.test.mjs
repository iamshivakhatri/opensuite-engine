...
test('reads and writes the same DOCX Buffer through the Rust engine', async () => {
  ...
  assert.equal(capabilities.formats[0].capabilities.length, 35)
  ...
  assert.ok(capabilities.formats[0].capabilities.includes('set_table_column_widths'))
  assert.ok(capabilities.formats[0].capabilities.includes('set_table_cell_shading'))
  ...
  assert.equal(new Set(capabilities.formats[0].capabilities).size, capabilities.formats[0].capabilities.length)

...
use opensuite_protocol::{
    ...
    PageBreakTarget, PageMargins, PageOrientation, PaperSize, ParagraphFormattingPatch,
    ParagraphListKind, ParagraphPlacement, PropertyPatch, ReplacePicture, ReplaceText,
    SetContentControlText, SetHeaderFooterText, SetHyperlink, SetPageSetup, SetParagraphFormatting,
    SetParagraphStyle, SetParagraphsList, SetPictureSize, SetTableCellText, SetTableCellsText,
...
const LETTER: (u32, u32) = (12_240, 15_840);
const A4: (u32, u32) = (11_906, 16_838);
const DEFAULT_MARGIN_TWIPS: i32 = 1_440;
const MAX_MARGIN_TWIPS: i32 = 31_680;
...
pub fn inspect_header_footer(
    ...
    Ok(simple_header_footer_text(&part_source, kind)
        .map(HeaderFooterInspection::SimpleText)
        .unwrap_or(HeaderFooterInspection::Unsupported))
}
...
pub fn set_header_footer_text_to_vec(
    ...
    if let Some(text) = &operation.text {
        if let Some(reference) = references.first().copied() {
            ...
            let bytes = package.read_part(&part).map_err(package_failure)?;
            let part_source = SourceDocument::parse(bytes).map_err(document_invalid)?;
            let patch = simple_header_footer_patch(&part_source, operation.kind, text)?;
    ...
    let output = package
        ...
        .map_err(package_failure)?;
    verify_header_footer_output(&output, operation, before_page_setup, opposite)?;
    Ok(output)
}
...
fn default_header_footer_references(
    source: &SourceDocument,
    section: NodeId,
    kind: HeaderFooterKind,
) -> Vec<NodeId> {
...
fn simple_header_footer_text(source: &SourceDocument, kind: HeaderFooterKind) -> Option<String> {
    let root = source.root();
    if !word(source, root, header_footer_root_name(kind)) {
        return None;
    }
    let paragraphs = source.children(root).collect::<Vec<_>>();
    if paragraphs.len() != 1 || !word(source, paragraphs[0], "p") {
        return None;
    }
    let runs = source
...
fn simple_header_footer_patch(
    source: &SourceDocument,
    kind: HeaderFooterKind,
    text: &str,
) -> Result<Patch, OperationResult> {
    simple_header_footer_text(source, kind)
        .ok_or_else(|| unsupported("header/footer is not a simple single-paragraph text part"))?;
    let value = source
        .children(
            source
                .children(source.children(source.root()).next().expect("paragraph"))
                .find(|id| word(source, *id, "r"))
                .expect("run"),
        )
        .find(|id| word(source, *id, "t"))
        .expect("text");
    let tag = qualified_name(source, value, "t")?;
...
fn next_header_footer_part_name(
    package: &Package,
    kind: HeaderFooterKind,
...
fn next_relationship_id(package: &Package, main: &Part) -> Result<String, OperationResult> {
    let used = package
        .part_relationships(main)
        .map_err(package_failure)?
        .into_iter()
...
fn relationship_xml(package: &Package, main: &Part) -> Result<(bool, Vec<u8>), OperationResult> {
...
fn header_footer_reference_xml(
    source: &SourceDocument,
    section: NodeId,
    kind: HeaderFooterKind,
    id: &str,
...
fn canonical_header_footer_xml(kind: HeaderFooterKind, text: &str) -> String {
...
fn verify_header_footer_output(
    ...
    let expected = operation
        .text
        .as_ref()
        .map_or(HeaderFooterInspection::None, |value| {
            HeaderFooterInspection::SimpleText(value.clone())
        });
    if actual != expected {
        return Err(OperationResult::failed(
            "DOCUMENT_INVALID",
...
fn main_section(source: &SourceDocument, allow_missing: bool) -> Result<NodeId, OperationResult> {
...
fn page_dimensions(source: &SourceDocument, size: NodeId) -> Result<(u32, u32), OperationResult> {
...
fn paper_dimensions(paper: PaperSize) -> (u32, u32) {
    match paper {
        PaperSize::Letter => LETTER,
        PaperSize::A4 => A4,
    }
}
fn paper_from_dimensions(width: u32, height: u32) -> Option<PaperSize> {
...
fn orientation_name(value: PageOrientation) -> &'static str {
    match value {
...
fn preserved_margin_attributes(
    source: &SourceDocument,
...
fn numbering_part_name(main: &Part) -> Result<PartName, OperationResult> {
    let directory = main
        .name
        .as_str()
        .rsplit_once('/')
...
fn numbering_ids(bytes: Option<&[u8]>) -> Result<(u32, u32), OperationResult> {
...
fn numbering_document(addition: &str) -> String {
    format!(
...
fn parse_table_handle(handle: &str) -> Result<usize, OperationResult> {
    handle
        .strip_prefix('t')
        .and_then(|value| value.parse().ok())
...
fn parse_row_handle(handle: &str) -> Result<(usize, usize), OperationResult> {
...
fn parse_cell_handle(handle: &str) -> Result<(usize, usize, usize), OperationResult> {
...
fn label_matches(actual: &str, requested: &str) -> bool {
    actual.trim() == requested.trim()
}

fn labels_match(actual: &[String], requested: &[String]) -> bool {
...
fn minimal_table_cell_fragment(
    source: &SourceDocument,
    row: NodeId,
    value: &str,
) -> Result<String, OperationResult> {

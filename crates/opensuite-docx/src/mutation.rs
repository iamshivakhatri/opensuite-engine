...
#[derive(Clone, Debug, Eq, PartialEq)]
struct SimpleHeaderFooterContent {
    text: Option<String>,
    page_number: Option<PageNumberAlignment>,
}

fn simple_header_footer_content(
    source: &SourceDocument,
    kind: HeaderFooterKind,
...
fn simple_header_footer_paragraph_text(
    source: &SourceDocument,
    paragraph: NodeId,
) -> Option<String> {
    let runs = source
...
fn header_footer_text_paragraph(source: &SourceDocument) -> Option<NodeId> {
    source
        .children(source.root())
        .find(|id| simple_header_footer_paragraph_text(source, *id).is_some())
}

fn page_number_paragraph(source: &SourceDocument) -> Option<NodeId> {
    source
        .children(source.root())
...
fn simple_header_footer_text_xml(
    source: &SourceDocument,
    kind: HeaderFooterKind,
    text: &str,
...
fn page_number_alignment_name(value: PageNumberAlignment) -> &'static str {
    match value {
...
fn canonical_page_number_header_footer_xml(
    kind: HeaderFooterKind,
    alignment: PageNumberAlignment,
) -> String {
    let root = header_footer_root_name(kind);
...
fn page_number_alignment_patch(
    source: &SourceDocument,
    alignment: PageNumberAlignment,
) -> Result<Patch, OperationResult> {
...
fn source_end_tag_start(source: &SourceDocument, id: NodeId) -> Result<usize, OperationResult> {
